...
LogItem * Loggerhandle::creatLogItem(const PGRFLoggerEventPkt  & testPaxoesEventPkt)
{
...
void Loggerhandle::saveItem(const PGRFLoggerEventPkt & pgp)
{
...
PGRFLoggerEventPkt Loggerhandle::getItem(U64 txid)
{
	PGRFLoggerEventPkt *myPkt = new  PGRFLoggerEventPkt();
	return *myPkt;
}
...
void Loggerhandle::fstr_creat( )
{
	...
	filename = temp +".dat";
	file.open(filename.c_str(),ios_base::binary|ios_base::out);//以写方式打开文件
	if(file.fail())
	{
		cout << "文件打开失败"<<endl;
		system("PAUSE");
    ...
    int temp = 0;
	 for(U32 i=myCircle->front;i< end;i++)
	 {
		...
		//将logitem，即myCircle中的一项读入文件
		  ...
		  cout <<  "key::" << myCircle->base[i]->key << "  value::" << myCircle->base[i]->value <<endl ;
...
void Loggerhandle::readfromfile()
{
	...
	logout.open(logfile.c_str(),ios::binary|ios::in);
	while(!logout)
	{
		cout<<"元文件打开失败"<<endl;
		system("PAUSE");
		logout.open(logfile.c_str(),ios::binary|ios::in);//以读写方式打开文件
	}
	...
	outCircle->CIQTraverse();//to test
	   ...
	   if(logout.tellg() >= ios_base::beg)
	   {
		  ...
		  cout<<logout.tellp()<<"  ";
		  logout.read(reinterpret_cast<char *>(item), sizeof(LogItem));
		  if(i>25)
		  cout<<i<<"  "<< logout.tellp()<<"  ";
			//if( > ios_base::beg)
			{
			...
			//outCircle->EnCICircle(item);
        }
	 
	   cout<<item->txid<<" "<<item->event_type <<" "<<item->key<<"  "<<item->value<<" "<<item->committed_flag<<endl;
...
int Loggerhandle::findItem(U64 find_txid)
{
	...
	while(!logout)
	{
		cout<<"元文件打开失败"<<endl;
		system("PAUSE");
		logout.open(logfile.c_str(),ios_base::binary|ios_base::in);//以读写方式打开文件
	}
	LogItem *li = new LogItem;
	logout.read(reinterpret_cast<char *>(li), sizeof(LogItem));
