...
void LoggerState::set_file_list(string filename,U64 txid_start,U64 txid_eof)
{
	...
	fstream metafile;
	metafile.open(log_metafile.c_str(),ios_base::binary|ios_base::app);//追加方式，向文件输出的内容追加到尾部
	while(!metafile)//当文件不存在时，新建一个文件
	{
		cout<<"元文件打开失败"<<endl;
		system("PAUSE");
		metafile.open(log_metafile.c_str(),ios_base::binary|ios_base::out);//新建一个文件
...
{
	fstream metafile;
	metafile.open(log_metafile.c_str(), ios_base::binary|ios_base::in);//以读写方式打开文件
	while(!metafile)
	{
		cout<<"元文件打开失败"<<endl;
		system("PAUSE");
